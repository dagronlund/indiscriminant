...
use syn::{parse_macro_input, Data, DeriveInput, Expr, Lit, LitByteStr, Visibility};

#[proc_macro_attribute]
pub fn no_discrimination_byte_str(args: TokenStream, input: TokenStream) -> TokenStream {
    ...
    let mut to_byte_str_matches = quote!();
    let mut from_byte_str_matches = quote!();

    for (variant_name, (discriminant, span)) in variants.clone() {
        let discriminant = LitByteStr::new(&discriminant, span);
        let variant_name = format_ident!("{}", variant_name);
        to_byte_str_matches.extend(quote! { #name::#variant_name => #discriminant, });
    ...
    match input.vis {
        Visibility::Public(_) => {
            result.extend(quote! {
                impl #name {
                    pub fn to_byte_str(&self) -> &'static [u8] { #to_byte_str_match }
                    pub fn from_byte_str(value: &[u8]) -> Self { #from_byte_str_match }
                }
            });
        }
        Visibility::Crate(_) => {
            result.extend(quote! {
                impl #name {
                    pub(crate) fn to_byte_str(&self) -> &'static [u8] { #to_byte_str_match }
                    pub(crate) fn from_byte_str(value: &[u8]) -> Self { #from_byte_str_match }
                }
            });
        }
        _ => {
            result.extend(quote! {
                impl #name {
                    fn to_byte_str(&self) -> &'static [u8] { #to_byte_str_match }
                    fn from_byte_str(value: &[u8]) -> Self { #from_byte_str_match }
                }
            });
        }
    }
